...
def analyze_resume(payload: AnalysisInput) -> dict[str, Any]:
    ...
    if provider in {"auto", "ollama"}:
        ai_result = _try_ollama_analysis(payload)
        if ai_result:
            return ai_result
...
def _try_ollama_analysis(payload: AnalysisInput) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        return _prepare_ai_result(data.get("response", "{}"), f"ai-ollama:{model}")
